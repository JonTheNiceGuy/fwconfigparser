...
foreach($lines as $line_num=>$line) {
  ...
  if(strtolower(trim($arr[0]))=='name') {
    ...
    $endpoint[trim($arr[1])][]=trim($arr[2]);
  }
  if(strtolower(trim($arr[0]))=='object-group') {
    $group['name']=trim($arr[2]);
  }
  if(strtolower(trim($arr[0]))=='network-object') {
  ...
  if(strtolower(trim($arr[0]))=='port-object') {
  ...
  if(strtolower(trim($arr[0]))=='access-list') {
    if(trim($arr[1])!='compiled') {
      ...
      if(trim($arr[6])=='host') {
        $toobj[$rule]='host';
        $tohost[$rule]=trim($arr[7]);
      } elseif (trim($arr[7])=='255.255.255.255') {
        $toobj[$rule]='host';
        $tohost[$rule]=trim($arr[6]);
      } elseif (trim($arr[6])=='object-group') {
        $toobj[$rule]='object';
        $tohost[$rule]=trim($arr[7]);
      } elseif (trim($arr[5])=='any') {
        $toobj[$rule]='host';
        $tohost[$rule]='Any Host';
      } else {
        $toobj[$rule]='host';
        $tohost[$rule]=getHostRange($arr[6], $arr[7]);
      }
      if($arr[3]=='icmp') {
        $portobj[$rule]='port';
        $porttxt[$rule]='Any ICMP';
      } elseif(strtolower(trim($arr[8]))=='eq') {
        $portobj[$rule]='port';
        $porttxt[$rule]=trim($arr[9]);
      } elseif(strtolower(trim($arr[8]))=='range') {
        $portobj[$rule]='port';
        if(is_numeric(trim($arr[9])) AND is_numeric(trim($arr[10]))) {
          for($port=trim($arr[9]); $port<=trim($arr[10]); $port++) {
            $porttxt[$rule][]=$port;
          }
        } else {
          for($port=9; isset($arr[$port]); $port++) {
            $porttxt[$rule][]=trim($arr[$port]);
          }
        }
      } elseif(strtolower(trim($arr[8]))=='object-group') {
        $portobj[$rule]='object';
        $porttxt[$rule]=trim($arr[9]);
      }
    }
